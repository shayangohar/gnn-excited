"""PyG dataset for QM-symex S/T pretraining (schema qmsymex-st-v1).

Targets are true-eV excitation energies (no rescaling needed); rows are keyed
by molecule_key with a status column per the train.py manifest contract.
"""
...
class QMSymexSTDataset(Dataset):
    '''PyG dataset over QM-symex HDF5 groups keyed by molecule_key.'''
    ...
    def __init__(
        ...
        if _IMPORT_ERROR is not None:
            raise ModuleNotFoundError(
                'QMSymexSTDataset requires torch and torch_geometric. Install the ML environment first.'
            ) from _IMPORT_ERROR
        super().__init__()
